...
def compute_spectra_by_second(data, sample_rate, apply_window=True,
                              n_bins=100, fmin=20.0, fmax=None, binning='log'):
    """
...
def piano_note_frequencies(midi_low=21, midi_high=108):
    """
...
def compute_note_magnitudes_per_second(data, sample_rate, notes=None,
                                       apply_window=True, n_harmonics=5,
                                       harmonic_weights=None,
                                       use_db=False, db_eps=1e-12, db_floor=None):
    """
...
def top_k_piano_notes_per_second(data, sample_rate, k=10, apply_window=True,
                                 use_db=False, db_eps=1e-12, db_floor=None):
    """
    Return a DataFrame where each row (second) lists the top-k piano notes and
    magnitudes.

    Columns: note_1, mag_1, note_2, mag_2, ..., note_k, mag_k
    """
...
def plot_spectrogram_heatmap(df, max_freq=None, log_scale=False,
                             clip_percentiles=(1, 99)):
    """
    Create a Plotly heatmap for the per-second spectrum DataFrame.

    Args:
        df: pandas.DataFrame produced by compute_spectra_by_second (rows=seconds, cols=freqs)
        max_freq: optional float, limit y-axis to frequencies <= max_freq (Hz)
    ...
    """
    # ensure columns are numeric frequencies
    freqs = np.array([float(f) for f in df.columns])
    seconds = df.index.values

    # optionally limit frequency range
    ...
    # z should be shape (n_freqs, n_seconds) so transpose the DataFrame
    z = data.T.values
    if log_scale:
        z_plot = np.log1p(z)
    else:
...
for file in song_data_files:
    ...
    # parse into piano note frequencies
    # get linear magnitudes (default)
    df_notes = compute_note_magnitudes_per_second(data, sample_rate)
    # get magnitudes in dB, clipped at -80 dB
    df_notes_db = compute_note_magnitudes_per_second(data, sample_rate,
    ...
    if data.shape[1] == 2:
        ...
        fig.add_trace(go.scatter(x=time, y=data, mode='lines',
                           name='Left Channel', line={'color':'blue'}), row=1, col=1)
    # plot the sound frequencies as a heatmap
    fig.add_trace(plot_spectrogram_heatmap(df, max_freq=None, log_scale=False,
                                           clip_percentiles=(1, 99)), row=2, col=1)
    
    # finish with the plot
    fig.update_layout(title_text=f'Audio Analysis for {song_id}', showlegend=True)

    plyoff.plot(fig, filename=os.path.join(PLOTS_PATH, '%s_waveform.html'%song_id),
                auto_open=False)
    # save it all
    songs[song_id] = (sample_rate, data, df, fig)
